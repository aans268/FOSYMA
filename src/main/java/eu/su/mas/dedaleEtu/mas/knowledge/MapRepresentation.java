...
public class MapRepresentation implements Serializable {
	...
	/**
	 * Retourne les id des noeuds voisins du noeud passé en arugment
	 * @param id : l'id du noeud
	 * @return Vecteur des id des noeuds voisins 
	 */
	public List<String> getNeighbours(String id){
		return new Vector<String>(this.g.getNode(id).neighborNodes().map(Node::getId).collect(Collectors.toList()));
	}
	...
	/**
	 * Trouve le chemin le vers le noeud ouvert le plus proche qui n'est pas bloqué par un autre agent.
	 * @param myPosition la position courante de l'agent.
	 * @param blockedPostions les positions (id) bloquées par les autres agents.
	 * @return l'ensemble des identifiants des noeuds du chemin ou null si
	 * aucun chemin accessible n'est trouvé.
	 */
	public List<String> getShortestPathToClosestOpenNode(String myPosition, Set<String> blockedPostions) {
		
		// La valeur du chemin le plus court
		int shortestValue = Integer.MAX_VALUE;
		
		// La liste correspondant au chemin le plus court
		List<String> shortestPath = null;
		
		// On récupère les noeuds ouverts
		List<String> openNodes = getOpenNodes();
		
		// On parcourt les noeuds ouverts
		for (String openNode : openNodes) {
			
			// On considère un noeud qui n'est pas bloqué
			if (!blockedPostions.contains(openNode)) {
				
				// On calcule le chemin le plus court de la position courante
				// vers ce noeud
				List<String> path = getShortestPath(myPosition, openNode);
				
				// Le nouveau chemin est plus court
				if (path != null && path.size() < shortestValue) {
					shortestPath = path;
					shortestValue = path.size();
				}
			}
		}
